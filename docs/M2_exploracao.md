...
* **Atributo PTRATIO vs. Alvo (MEDV):** A variável PTRATIO surge como o terceiro preditor com maior relação com a variável alvo MEDV, apresentando uma correlação negativa moderada de r = 0.51, evidenciada tanto no heatmap de correlação como no scatter plot. Bairros com rácios mais elevados de alunos por professor, indicativos de menor qualidade no ensino público, estão associados a valores medianos de habitação mais reduzidos. Este resultado demonstra que o valor imobiliário no dataset de Boston é moldado em grande parte pelo perfil dos serviços e do ambiente do bairro, confirmando que fatores socioeconómicos e de qualidade urbana superam as características físicas da própria habitação na determinação do preço.
