...
| DBSCAN | `eps=0.5`, `min_samples=5`,`metric='euclidean'`| 0.2332 | 0.3459 | 110.3760 | 109.8801 | 0.7510 | 0.6706 | 10 | 4 | 112 | 56 | _Silhouette_ abaixo de 0,50 em ambos os conjuntos, elevado número de pontos de ruído e número de clusters inconsistente entre treino e teste  |
...
O **Agrupamento Aglomerativo**, com parâmetros por omissão (`linkage='ward'`, `n_clusters=2`e `metric='euclidean'`), apresentou a _Silhouette_ mais elevada no conjunto de teste (0,5868) entre todos os modelos avaliados, superando o objetivo SMART e sem gerar qualquer ponto de ruído, sendo por isso selecionado como modelo final. Contudo, a queda no índice _Calinski-Harabasz_ do treino para o teste (125,97 vs 90,45) representa uma limitação moderada do ponto de vista da robustez metodológica.
O **DBSCAN**, com os parâmetros por omissão (eps=0.5, min_samples=5 e metric='euclidean'), registou valores de _Silhouette_ abaixo do objetivo SMART em ambos os conjuntos (0,2332 no treino e 0,3459 no teste). A instabilidade é evidente no número de clusters obtidos (10 no treino vs 4 no teste) e no elevado volume de ruído (112 observações no treino e 56 no teste), o que inviabiliza a sua adoção como modelo final.
...
### 4.3. Problema 2- Caracterização dos _CLusters_
O modelo final (_Agglomerative Clustering_ com `linkage='complete'` e `n_clusters=2`) identificou dois segmentos distintos no conjunto de dados: segmento 0 com 487 observações e o segmento 1 com 19 observações.
O Segmento 0 caracteriza-se por habitações em zonas predominantemente urbanas, com baixa percentagem de zona residencial (ZN=8,78), elevada presença industrial (INDUS=11,49), habitações mais antigas (AGE=70,00) e taxa de imposto mais elevada (TAX=413,03). O índice de qualidade de vida (IQV=9,25) é substancialmente inferior ao do Segmento 1.
O Segmento 1, apesar de minoritário (19 observações), representa habitações em zonas residenciais de maior dimensão (ZN=77,66), com menor presença industrial (INDUS=2,15), habitações mais recentes (AGE=32,06), menor carga fiscal (TAX=285,47) e um índice de qualidade de vida bastante superior (IQV=74,05).
Com base nas características identificadas, é possível atribuir uma designação interpretativa a cada segmento: o Segmento 0 pode ser denominado "Zona Urbana Desvalorizada", correspondendo a habitações mais antigas, em zonas industriais e com menor qualidade de vida; o Segmento 1 pode ser designado "Zona Residencial Valorizada", representando habitações mais recentes, em zonas amplas e com índices de qualidade de vida substancialmente superiores. 
...
No que respeita ao Objetivo SMART 1, previsão do valor mediano das habitações `MEDV`, o modelo final selecionado foi o _Random Forest_,pois este apresenta um R² mais elevado (0.83) e um RMSE mais baixo(3.59) em comparação com os outros modelos analisados. Porém, após a otimização de hiperparâmetros via _GridSearchCV_ com validação cruzada de 5 _folds_, o modelo não registou melhorias de desempenho, o que indica que os hiperparâmetros por omissão já se encontravam próximos do ideal. Em seguida, procedeu-se à validação cruzada K-Fold (k=10), onde revelou um RMSE médio de 4,026, um R² médio de 0,799 e um MAE de 2,812, valores ligeiramente inferiores aos obtidos no conjunto de teste, o que indica que o modelo apresenta alguma sensibilidade à partição dos dados mas mantém um desempenho globalmente aceitável.

No que respeita ao Objetivo SMART 2, segmentação dos bairros de Boston, o modelo final selecionado foi o _Agglomerative Clustering_ (linkage='complete', n_clusters=2), que atingiu um Coeficiente de Silhueta de 0,7363 no conjunto de teste, superando largamente o critério mínimo definido (>0,50). A estabilidade do modelo foi confirmada através de uma análise de Bootstrap com 30 iterações, com um intervalo de confiança a 95% de [0,530; 0,755], garantindo que o critério mínimo é cumprido independentemente da amostra utilizada. Os dois segmentos identificados, "Zona Urbana Desvalorizada" e "Zona Residencial Valorizada", apresentam perfis distintos e interpretáveis, reforçando a relevância prática da segmentação obtida.

Em síntese, o modelo final do Problema 1 cumpre parcialmente os critérios de sucesso estabelecidos no Objetivo SMART 1: o R² de 0,83 supera o limiar mínimo definido (> 0,80), contudo o RMSE de 3,59 não cumpre o critério estabelecido (< 3.500$), ficando ligeiramente acima do valor pretendido. O modelo do Problema 2 cumpre integralmente o critério definido no Objetivo SMART 2, com um Silhouette Score de 0,7363 no conjunto de teste, largamente superior ao limiar mínimo (> 0,50). As principais limitações identificadas prendem-se com a dimensão reduzida do dataset (506 observações) e a natureza histórica dos dados (recolhidos em 1978), fatores que poderiam ser mitigados com uma maior quantidade de dados e informação mais recente em trabalhos futuros.
